//this is an array/list as denoted by []
const tracks = [
    {
...
//   //add this to the main and append the template to it
//   document.querySelector('main').innerHTML += template;

let index=0;

//for -- in -- loop (always use const because do not want to modify what you are working with)
for (const track of tracks) {
  const template = 
        //want to set each data-index unique for each card so that can play the song at that index
  ...
  //increase index by one each time
  index+=1;
}

//Second version of for loop (more similar to other langauges). Good if you are looping
//through a known set of values. Can increment or decrement 
//In the first version, increment over the list directly whereas here we are 
//incrementing over the indices of the list
...
//event gets passed in to the function based on what occured
const playSong = (ev) => {
    //currentTarget shows where the user is clicking
    //console.log(ev.currentTarget.dataset);

    //Get data-set index of the card that is clicked and store this as a number
    let song_index = Number(ev.currentTarget.dataset.index);

    //this gets the MP3 player for the song

    //selecting the audio player elment and set equal to the MP3 of the song
